...
| Startup live-eval proof manifest | Added | `docs/eval/startup-diligence-war-room-live.json`, `docs/eval/startup-diligence-war-room-live.md` | "The repo distinguishes live shell proof, deterministic UI proof, and pending live-provider checks." |
| Runway/milestone package | Helper built; room workflow pending | `src/nodeagent/skills/finance/runwayForecaster.ts`, `src/nodeagent/components/RunwayChart.tsx`, `tests/nodeagentAlignment.test.ts`, `docs/eval/startup-diligence-war-room-live.json` | "Runway math and chart rendering exist; live room artifact generation still gates production proof." |
