...
| Proposal approval away from the cell | Added inline approve/reject controls on cells with pending proposals; trace strip remains the audit and bulk-accept path. Review-mode agents now treat pending approval as a handoff and duplicate pending writes are coalesced. | `src/ui/panels/Artifact.tsx`, `src/app/styles.css`, `src/nodeagent/core/plans.ts`, `src/engine/roomEngine.ts` |
