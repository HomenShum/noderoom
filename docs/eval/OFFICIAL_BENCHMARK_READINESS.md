...
Generated: 2026-06-13T10:28:39.026Z
...
Blockers:
- official_gold_isolation: SpreadsheetBench staging separates agent-visible files from evaluator-only gold/scorer metadata; one passing fixture smoke and one official V1 failed-path model smoke exist, but official full-bundle sandboxing and output policy proof are still missing.
...
Blockers:
- official_gold_isolation: SpreadsheetBench staging separates agent-visible files from evaluator-only gold/scorer metadata; model-edit smoke evidence exists for V1, but chart/visual grading and process sandboxing are still missing.
- official_runner_adapter: A copy-input baseline, deterministic edit-plan worker, and model-edit-plan worker emit candidate workbooks from staged agent directories and score them afterward; V1 model smoke evidence exists, but V2 official runs, benchmark policy, retries, process sandbox proof, and chart lane remain incomplete.
- formula_recompute: Finance eval recomputes supported formulas; full Excel-compatible official recompute is not complete.
